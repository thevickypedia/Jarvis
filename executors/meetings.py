...
def meetings_gatherer() -> str:
    ...
    if not os.path.isfile(MEETING_FILE):
        return f"I wasn't able to find the meetings script for your {env.meeting_app} sir!"
    failure = None
    ...
    if error := process.returncode:  # stores non zero error
        ...
        if err_code == '(-1728)':  # If 'Office' is unavailable in calendar/outlook application
            ...
            return f"Office is unavailable in your {env.meeting_app} sir!"
        ...
        return f"I was unable to read your {env.meeting_app} sir! Please make sure it is in sync."
...
def meetings():
    ...
    if os.path.isfile('meetings'):
        ...
        speaker.speak(text=f"Please give me a moment sir! Let me check your {env.meeting_app}.", run=True)
        try:
