            ...
            if any(re.search(line, target['name'], flags=re.IGNORECASE) for line in phrase.split()):
                ...
                if 'disconnect' in phrase:
                    ...
                    if not output:
                        ...
                        speaker.speak(text=f"Disconnected from {target['name']} {models.env.title}!")
                    else:
                        speaker.speak(text=f"I was unable to disconnect {target['name']} {models.env.title}!. "
                                           f"Perhaps it was never connected.")
                elif 'connect' in phrase:
                    output = subprocess.getoutput(cmd=f"blueutil --connect {target['address']}")
                    if not output:
                        time.sleep(2)  # included a sleep here, so it avoids voice swapping between devices
                        speaker.speak(text=f"Connected to {target['name']} {models.env.title}!")
                    else:
                        speaker.speak(text=f"Unable to connect {target['name']} {models.env.title}!, "
...
def bluetooth(phrase: str) -> None:
    ...
    if 'turn off' in phrase or 'power off' in phrase:
        ...
        speaker.speak(text=f"Bluetooth has been turned off {models.env.title}!")
        ...
        speaker.speak(text=f"Bluetooth has been turned on {models.env.title}!")
    elif 'disconnect' in phrase and ('bluetooth' in phrase or 'devices' in phrase):
        subprocess.call("blueutil --power 0", shell=True)
        time.sleep(2)
        subprocess.call("blueutil --power 1", shell=True)
        speaker.speak(text=f"All bluetooth devices have been disconnected {models.env.title}!")
    else:
        ...
        if not connector(phrase=phrase, targets=paired):
            ...
            speaker.speak(text=f"No connections were established {models.env.title}, looking for un-paired devices.",
                          run=True)
            ...
            connector(phrase=phrase, targets=unpaired) if unpaired else \
                speaker.speak(text=f"No un-paired devices found {models.env.title}! You may want to be more precise.")
