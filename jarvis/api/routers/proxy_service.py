...
async def proxy_service_api(request: Request, origin: HttpUrl, output: str):
    ...
    try:
        ...
        if response.ok:
            ...
            raise APIResponse(status_code=HTTPStatus.BAD_REQUEST.real, detail=HTTPStatus.BAD_REQUEST.phrase)
        raise APIResponse(status_code=response.status_code, detail=response.text)
    except EgressErrors as error:
        logger.error(error)
        raise APIResponse(status_code=HTTPStatus.SERVICE_UNAVAILABLE.real, detail=HTTPStatus.SERVICE_UNAVAILABLE.phrase)
