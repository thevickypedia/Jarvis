    ...
    with models.db.connection as connection:
        ...
        for attr in models.tables.model_fields:
            ...
            logger.info(
                ...
                table,
                util.matrix_to_flat_list([list(filter(None, d)) for d in data if any(d)]),
            )
            cursor.execute(f"DELETE FROM {table.name}")


# noinspection LongLine
def create_process_mapping(processes: Dict[str, Dict[str, Process | List[str]]], func_name: str = None) -> None:
    ...
    if func_name:
        ...
        dump[func_name] = {processes[func_name]["process"].pid: processes[func_name]["impact"]}
    else:
        dump = {k: {v["process"].pid: processes[k]["impact"]} for k, v in processes.items()}
...
def start_processes(func_name: str = None) -> Process | Dict[str, Process]:
    ...
    if func_name:
        ...
        processes: Dict[str, Process] = {func: process_dict[func]["process"] for func in process_dict.keys()}
